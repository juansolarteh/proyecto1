...
public class PracticesServiceImpl extends GenericServiceImpl<Practices, PracticesDTO> implements PracticesServiceAPI {
	...
	public String crearCSV(String idResultado) throws Exception {
		...
		try {
			...
			for (PracticesDTO practice : getAll()) {
				if (practice != null) {
					if (practice.getId().compareTo(idResultado) == 0) {
						...
						SimpleDateFormat formato1 = new SimpleDateFormat("yyyy-MM-dd");
						SimpleDateFormat formato2 = new SimpleDateFormat("HH:mm:ss");
						String fechaInicio = formato1.format(practice.getStart()) + "  "
								+ formato2.format(practice.getStart());
						String fechaFin = formato1.format(practice.getEnd()) + "  "
								+ formato2.format(practice.getEnd());
						...
						if (topic.compareTo("Ley de Hooke") == 0) {
							...
							salidaCSV.write("*************Pesos Obtenidos por la maquina***********************");
							salidaCSV.endRecord();
							for (String key : practice.getData().get("Pesos_maquina").keySet()) {
								salidaCSV.write(key);
								salidaCSV.write(practice.getData().get("Pesos_maquina").get(key).toString());
								salidaCSV.endRecord();
							}
							salidaCSV.write("*************Pesos Seleccionados por el Estudiante***********************");
							salidaCSV.endRecord();
							for (String key : practice.getData().get("peso").keySet()) {
								salidaCSV.write(key);
								salidaCSV.write(practice.getData().get("peso").get(key).toString());
								salidaCSV.endRecord();
							}
			...
			URL url = generateV4GetObjectSignedUrl("proyecto-1-94c3c", "proyecto-1-94c3c.appspot.com",
					idResultado + ".csv");
	...
	private void subirArchivo(String projectId, String bucketName, String objectName, String filePath)
			throws IOException {
		...
		StorageOptions storageOptions = StorageOptions.newBuilder().setProjectId(projectId)
				.setCredentials(GoogleCredentials.fromStream(new FileInputStream("./ClaveProyecto.json"))).build();
	...
	private URL generateV4GetObjectSignedUrl(String projectId, String bucketName, String objectName)
		...
		StorageOptions storageOptions = StorageOptions.newBuilder().setProjectId("proyecto-1-94c3c")
				.setCredentials(GoogleCredentials.fromStream(new FileInputStream("./ClaveProyecto.json"))).build();
		Storage storage = storageOptions.getService();
	...
	public void datosSeleccionados(String planta,String repeticion,Map<String,Float> datos) throws Exception{
		...
		DocumentSnapshot document=future.get();
		if(document.exists()) {
			idPractice = document.getData().get(planta).toString();
		}
		if(idPractice.compareTo("")!=0 || idPractice==null) {		
			DocumentReference docRef2=firestore.collection("Practices").document(idPractice);
			Map<String,Map<String,Float>> data = get(idPractice).getData();
			for (String dato :datos.keySet()) {
				Map<String,Float> variable = data.get(dato);
				if(variable==null) {
					variable = new HashMap<>();
				}
				variable.put(repeticion, datos.get(dato));
				data.put(dato, variable);
			}
