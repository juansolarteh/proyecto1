...
public abstract class GenericServiceImpl<I, O> implements GenericServiceAPI<I, O> {
	...
	public List<O> getListByFieldPath(String fieldPath, Object value) throws Exception {
		List<O> result = new ArrayList<O>();
		CollectionReference ref = getCollection();
		Query query = ref.whereEqualTo(fieldPath, value);
	...
	@SuppressWarnings("unchecked")
	public List<O> getFromMapByKey(String nameMap, Object key) throws Exception {
		List<O> result = new ArrayList<O>();
		ApiFuture<QuerySnapshot> query = getCollection().get();
		List<QueryDocumentSnapshot> documents = query.get().getDocuments();
		Map<O,O> map;
		for (QueryDocumentSnapshot doc : documents) {
			O object = doc.toObject(clazz);
			map = (Map<O, O>) PropertyUtils.getProperty(object, nameMap);
	...
	public O getByFieldPath(String fieldPath, Object value) throws Exception {
		CollectionReference ref = getCollection();
		Query query = ref.whereEqualTo(fieldPath, value).limit(1);
		ApiFuture<QuerySnapshot> querySnapshot = query.get();
		List<QueryDocumentSnapshot> documents = querySnapshot.get().getDocuments();
		for (DocumentSnapshot document : documents) {
			if (document.exists()) {
